...
int main(int argc, char **argv) {
    ...
    const int num_rot = 8; // because flip
    ...
    img_small_blocks_transformations[0] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[1] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[2] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[3] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[4] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[5] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[6] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    img_small_blocks_transformations[7] = static_cast<float*>(_mm_malloc(img_small_num_blocks * block_num_pixels * sizeof(float), 32));
    ...
    //split_into_blocks(             block_num_rows, block_num_cols, height_small, width_small, img_small,              img_small_blocks_transformations[0]);
    //split_into_blocks_rot90(       block_num_rows, block_num_cols, height_small, width_small, img_small_rot90,        img_small_blocks_transformations[1]);
    //split_into_blocks_rot180(      block_num_rows, block_num_cols, height_small, width_small, img_small_rot180,       img_small_blocks_transformations[2]);
    //split_into_blocks_rot270(      block_num_rows, block_num_cols, height_small, width_small, img_small_rot270,       img_small_blocks_transformations[3]);
    //split_into_blocks_hflip(       block_num_rows, block_num_cols, height_small, width_small, img_small_hflip,        img_small_blocks_transformations[4]);
    //split_into_blocks_hflip_rot90( block_num_rows, block_num_cols, height_small, width_small, img_small_hflip_rot90,  img_small_blocks_transformations[5]);
    //split_into_blocks_hflip_rot180(block_num_rows, block_num_cols, height_small, width_small, img_small_hflip_rot180, img_small_blocks_transformations[6]);
    //split_into_blocks_hflip_rot270(block_num_rows, block_num_cols, height_small, width_small, img_small_hflip_rot270, img_small_blocks_transformations[7]);
    split_into_blocks(             block_num_rows, block_num_cols, height,       width,       img_gray,               img_gray_blocks);
    ...
    free(img_small_hflip);
    free(img_small_hflip_rot90);
    free(img_small_hflip_rot180);
    free(img_small_hflip_rot270);
    std::cout << "\tDone." << std::endl;
    
    std::cout << "\tStart: find optimal mapping" << std::endl;
    find_optimal_mappings_flip(block_num_rows,
            block_num_cols,
            img_gray_num_blocks,
    ...
    std::cout << "Start: write fic_compress_flip_fast_output" << std::endl;
    std::ofstream ofile((resolver.resolve("images").make_absolute()/filesystem::path("fic_compress_flip_fast_output")).str());
    ...
    unsigned char *img_small_hflip_int               = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_hflip_rot90_int         = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_hflip_rot180_int        = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_hflip_rot270_int        = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    ...
    unsigned char *img_small_blocks_hflip_int        = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_blocks_hflip_rot90_int  = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_blocks_hflip_rot180_int = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    unsigned char *img_small_blocks_hflip_rot270_int = (unsigned char *)malloc(height_small*width_small*sizeof(unsigned char));
    for (int idx = 0; idx < height_small*width_small; idx++) {
        ...
        img_small_hflip_int[idx]               = (uint8_t)img_small_hflip[idx];
        img_small_hflip_rot90_int[idx]         = (uint8_t)img_small_hflip_rot90[idx];
        img_small_hflip_rot180_int[idx]        = (uint8_t)img_small_hflip_rot180[idx];
        img_small_hflip_rot270_int[idx]        = (uint8_t)img_small_hflip_rot270[idx];
        img_small_blocks_int[idx]              = (uint8_t)img_small_blocks_transformations[0][idx];
        img_small_blocks_rot90_int[idx]        = (uint8_t)img_small_blocks_transformations[1][idx];
        img_small_blocks_rot180_int[idx]       = (uint8_t)img_small_blocks_transformations[2][idx];
        img_small_blocks_rot270_int[idx]       = (uint8_t)img_small_blocks_transformations[3][idx];
        img_small_blocks_hflip_int[idx]        = (uint8_t)img_small_blocks_transformations[4][idx];
        img_small_blocks_hflip_rot90_int[idx]  = (uint8_t)img_small_blocks_transformations[5][idx];
        img_small_blocks_hflip_rot180_int[idx] = (uint8_t)img_small_blocks_transformations[6][idx];
        img_small_blocks_hflip_rot270_int[idx] = (uint8_t)img_small_blocks_transformations[7][idx];
    }
    for (int idx = 0; idx < height*width; idx++) {
    ...
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_hflip_rot0.png")).str().c_str(),   width_small,  height_small, 1, img_small_hflip_int,        width_small);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_hflip_rot90.png")).str().c_str(),  height_small, width_small,  1, img_small_hflip_rot90_int,  height_small);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_hflip_rot180.png")).str().c_str(), width_small,  height_small, 1, img_small_hflip_rot180_int, width_small);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_hflip_rot270.png")).str().c_str(), height_small, width_small,  1, img_small_hflip_rot270_int, height_small);
    ...
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_small_blocks_hflip.png")).str().c_str(), block_num_cols, img_small_num_blocks * block_num_rows, 1, img_small_blocks_hflip_int, block_num_cols);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_small_blocks_hflip_rot90.png")).str().c_str(), block_num_cols, img_small_num_blocks * block_num_rows, 1, img_small_blocks_hflip_rot90_int, block_num_cols);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_small_blocks_hflip_rot180.png")).str().c_str(), block_num_cols, img_small_num_blocks * block_num_rows, 1, img_small_blocks_hflip_rot180_int, block_num_cols);
    stbi_write_png((resolver.resolve("images").make_absolute()/filesystem::path("test_small_blocks_hflip_rot270.png")).str().c_str(), block_num_cols, img_small_num_blocks * block_num_rows, 1, img_small_blocks_hflip_rot270_int, block_num_cols);
    free(img_small_int);
    free(img_small_rot90_int);
    free(img_small_rot180_int);
    free(img_small_rot270_int);
    free(img_small_hflip_int);
    free(img_small_hflip_rot90_int);
    free(img_small_hflip_rot180_int);
    free(img_small_hflip_rot270_int);
    ...
    free(img_small_blocks_hflip_int);
    free(img_small_blocks_hflip_rot90_int);
    free(img_small_blocks_hflip_rot180_int);
    free(img_small_blocks_hflip_rot270_int);
    std::cout << "Done.\n" << std::endl;
    */
    //-----------------------------------DEBUGGING--------------------------------------------------------------------------------
    ...
    _mm_free(img_gray_blocks);
    _mm_free(img_small_blocks_transformations[0]);
    _mm_free(img_small_blocks_transformations[1]);
    _mm_free(img_small_blocks_transformations[2]);
    _mm_free(img_small_blocks_transformations[3]);
    _mm_free(img_small_blocks_transformations[4]);
    _mm_free(img_small_blocks_transformations[5]);
    _mm_free(img_small_blocks_transformations[6]);
    _mm_free(img_small_blocks_transformations[7]);
    delete[] img_small_blocks_transformations;
