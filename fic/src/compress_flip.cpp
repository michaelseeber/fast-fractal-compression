...
#include <fic/map_one_matrix_to_another_flip.h>
#include <fic/mapping_between_two_matrices_flip.h>
#include <fstream>
#include <chrono>
...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < blockCount16; i++) {    
        for(int j = 0; j < blockCount8; j++) {
            mapping_between_two_matrices_with_flip(blockDimension8, blockStore16[i], blockStore8[j], mapArray);
            map_one_matrix_to_another_with_flip(blockDimension8, blockStore16[i], mapArray, dataArray);
            double error;
    ...
    std::cout << "Start: write fic_compress_flip_output" << std::endl;
    std::ofstream ofile((resolver.resolve("images").make_absolute()/filesystem::path("fic_compress_flip_output")).str());
